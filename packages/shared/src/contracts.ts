...
export interface RegisterUserRequest { name: string; team?: string; group?: string; }
...
export interface EventApi { registerUser(input: RegisterUserRequest): Promise<User>; getQuestion(id: number): Promise<QuestionResponse>; submitAnswer(id: number, input: SubmitAnswerRequest): Promise<SubmitAnswerResponse>; getRanking(scope?: RankingScope, filter?: string): Promise<RankingResponse>; getStats(): Promise<Stats>; }
