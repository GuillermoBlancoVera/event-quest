...
export function PlayerProfile() {
  ...
  return <section className="game-panel"><div className="profile-hero"><div className="profile-id"><div className="avatar">{profile.name.slice(0, 1).toUpperCase()}</div><div><p className="game-kicker">JUGADOR</p><h1>{profile.name}</h1><p>{profile.team} · {profile.group}</p></div></div><div><p className="game-kicker">PUNTOS</p><div className="score">{profile.score}</div></div></div><div className="profile-grid"><article className="profile-card"><h2>Retos completados</h2>{profile.history.length ? <ul className="challenge-history">{profile.history.map(item => <li key={item.challengeId}><span>{item.title}</span><strong>+{item.points}</strong></li>)}</ul> : <p>Aún no has completado retos. El primer QR te está esperando.</p>}</article><article className="profile-card"><h2>Tu partida</h2><p><strong>{profile.completedChallenges.length}</strong> retos jugados</p><p><Link to="/juego/reto/1">Ir al reto activo →</Link></p><p><Link to="/juego/clasificacion">Ver clasificación →</Link></p></article></div></section>;
}
