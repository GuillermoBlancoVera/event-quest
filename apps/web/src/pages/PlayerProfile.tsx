...
export function PlayerProfile() {
  ...
  const logout = () => { localStorage.removeItem('event-quest-user'); localStorage.removeItem('event-quest-session-token'); navigate('/'); };

  return <><section className="game-panel profile-summary"><button className="profile-qr-button" onClick={() => setShowQr(true)} aria-label="mostrar qr del perfil"><img src={qrCodeIcon} alt="" /></button><div className="profile-hero"><div className="avatar profile-main-avatar">{avatarUrl && !avatarFailed ? <img src={avatarUrl} alt={`Foto de ${profile.name}`} onError={() => setAvatarFailed(true)} /> : profile.name.slice(0, 1).toUpperCase()}</div><h1 className="profile-name">{profile.name}</h1><div className="profile-summary-details"><Link className="profile-summary-column profile-summary-link" to="/juego/clasificacion"><p className="game-kicker">POSICIÓN</p><div className="profile-ranking-value">{playerRanking ? `${playerRanking.rank}${playerRanking.gender === 'female' ? 'ª' : 'º'}` : '—'}</div></Link><div className="profile-summary-column"><p className="game-kicker">COMUNIDAD/CABAÑA</p><div className="profile-affiliations">{playerRanking?.parentAffiliation && <RankingAffiliationIcon name={playerRanking.parentAffiliation.name} iconKey={playerRanking.parentAffiliation.avatarKey} to={`/juego/afiliacion/${playerRanking.parentAffiliation.affiliationId}`} />}{playerRanking?.parentAffiliation && playerRanking?.affiliation && <span className="profile-affiliation-separator" aria-hidden="true">/</span>}{playerRanking?.affiliation && <RankingAffiliationIcon name={playerRanking.affiliation.name} iconKey={playerRanking.affiliation.avatarKey} to={`/juego/afiliacion/${playerRanking.affiliation.affiliationId}`} />}</div></div><div className="profile-summary-column"><p className="game-kicker">PUNTOS</p><div className="score">{profile.score}</div></div><Link className="profile-summary-column profile-summary-link" to="/juego/retos"><p className="game-kicker">PROGRESO DE RETOS</p><div className="profile-progress-circle" style={progressStyle}><span>{attemptedChallenges} / {stats?.challenges ?? '—'}</span></div></Link></div></div></section><button className="profile-logout" onClick={logout}>Cerrar sesión</button>{showQr && <div className="profile-qr-modal" role="dialog" aria-modal="true" aria-label="qr del perfil"><div><button className="profile-qr-close" onClick={() => setShowQr(false)} aria-label="cerrar">×</button><p>qr de {profile.name}</p><QRCodeSVG value={qrValue} size={240} level="M" includeMargin /><button className="button" onClick={() => setShowQr(false)}>cerrar</button></div></div>}</>;
}
