...
export function PlayerProfile() {
  ...
  return <section className="game-panel"><div className="profile-hero"><div className="profile-id"><div className="avatar">{avatarUrl && !avatarFailed ? <img src={avatarUrl} alt={`Foto de ${profile.name}`} onError={() => setAvatarFailed(true)} /> : profile.name.slice(0, 1).toUpperCase()}</div><div><h1>{profile.name}</h1><div className="profile-affiliations">{playerRanking?.affiliation && <RankingAffiliationIcon name={playerRanking.affiliation.name} iconKey={playerRanking.affiliation.avatarKey} to={`/juego/afiliacion/${playerRanking.affiliation.affiliationId}`} />}{playerRanking?.parentAffiliation && <RankingAffiliationIcon name={playerRanking.parentAffiliation.name} iconKey={playerRanking.parentAffiliation.avatarKey} to={`/juego/afiliacion/${playerRanking.parentAffiliation.affiliationId}`} />}</div></div></div><div><p className="game-kicker">PUNTOS</p><div className="score">{profile.score}</div></div></div><div className="tabs profile-tabs"><button className={activeTab === 'challenges' ? 'active' : ''} onClick={() => setActiveTab('challenges')}>Retos</button><button className={activeTab === 'ranking' ? 'active' : ''} onClick={() => setActiveTab('ranking')}>Clasificación</button></div>{activeTab === 'challenges' ? profile.history.length ? <ul className="challenge-history profile-list">{profile.history.map(item => { const points = item.awardedPoints; const tone = points > 0 ? 'positive' : points < 0 ? 'negative' : 'zero'; return <li key={item.challengeId}><Link className="profile-challenge-link" to={`/juego/reto/${item.challengeId}`}><span>{item.title}</span><strong className={`challenge-points ${tone}`}>{points > 0 ? `+${points}` : points}</strong></Link></li>; })}</ul> : <p>Aún no has jugado retos. El primer QR te está esperando.</p> : !ranking ? <p>Actualizando clasificación…</p> : <ol className="ranking profile-ranking">{ranking.entries.map(entry => { const affiliation = entry.parentAffiliation ?? entry.affiliation; return <li className="profile-ranking-item" key={entry.userId}><b>{entry.rank}</b><RankingAvatar entry={entry} /><span>{entry.name}</span>{affiliation && <RankingAffiliationIcon name={affiliation.name} iconKey={affiliation.avatarKey} to={`/juego/afiliacion/${affiliation.affiliationId}`} />}<strong>{entry.score}</strong></li>; })}</ol>}</section>;
}
