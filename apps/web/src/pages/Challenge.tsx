...
export function Challenge() {
  ...
  return <Page eyebrow={question?.title ?? 'Cargando reto'}>{!question ? <p>{message || 'Desvelando la pista…'}</p> : <><p className="lead">{question.question}</p>{(resultMessage || message) && <p className="notice">{resultMessage || message}</p>}<div className="answers">{question.answers.map(item => { const correct = Boolean(answered && question.correctAnswer === item); const incorrect = Boolean(answered && !attempt?.correct && attempt?.answer === item); const state = correct ? 'selected answer-correct' : incorrect ? 'selected answer-incorrect' : answer === item && !answered ? 'selected' : ''; return <button className={state} disabled={answered} onClick={() => setAnswer(item)} key={item}>{item}</button>; })}</div><button className="button" disabled={(!answer && !answered) || busy} onClick={submit}>{busy ? 'Enviando…' : answered ? 'Volver a mi perfil' : 'Enviar respuesta'}</button></>}</Page>;
}
