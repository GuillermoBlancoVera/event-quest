...
export function PlayerChallenges() {
  ...
  return <Page eyebrow="Retos" top={<Link className="game-back-link" to="/juego/perfil">← Mi perfil</Link>}><ul className="challenge-history profile-list">{challenges.map(challenge => { const attempt = attempts.get(challenge.challengeId); if (!attempt) return <li className="profile-challenge-locked" key={challenge.challengeId}><span>{challenge.title}</span><span className="challenge-lock" aria-label="Reto bloqueado">🔒</span></li>; const points = attempt.awardedPoints; const tone = points > 0 ? 'positive' : points < 0 ? 'negative' : 'zero'; return <li key={challenge.challengeId}><Link className="profile-challenge-link" to={`/juego/reto/${challenge.challengeId}`}><span>{challenge.title}</span><strong className={`challenge-points ${tone}`}>{points > 0 ? `+${points}` : points}</strong></Link></li>; })}</ul></Page>;
}
